import { getHistory, getNewsletter } from "@/sanity/sanity-utils";
...
const components: PortableTextComponents = {
  ...
  block: {
    ...
    h1: ({ children }) => (
      <h1 className="text-3xl text-center font-extrabold text-center mt-6">{children}</h1>
    ),
    h2: ({ children }) => (
      <h2 className="text-2xl text-center font-bold text-center mt-4">{children}</h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-xl text-center font-semibold text-center mt-3">{children}</h3>
    ),
    h4: ({ children }) => (
      <h4 className="text-lg text-center font-medium text-center mt-2">{children}</h4>
    ),
    h5: ({ children }) => (
      <h5 className="text-base text-center font-medium text-center mt-2">{children}</h5>
    ),
    h6: ({ children }) => (
      <h6 className="text-sm text-center font-normal text-center mt-2">{children}</h6>
    ),
    blockquote: ({ children }) => (
...
type Props = {
  ...
  export default async function History(props: Props) {
    const { history: slug } = await props.params; 
      ...
      <div className="min-h-screen bg-[#f8f8f6] text-[#333]">
        ...
        <main className="text-center opacity-100">
          ...
          <div className="mt-8 mx-auto text-left leading-relaxed text-[#434343] max-w-7xl">
            <PortableText value={history.content} components={components} />
          </div>
        </main>
      </div>
    );
